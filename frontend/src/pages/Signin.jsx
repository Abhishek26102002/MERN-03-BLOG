...
export default function Signin(props) {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const response = await fetch(`https://mern-03-blog-production.up.railway.app/api/userlogin`, {
        method: 'POST',
        headers: {
      ...
      if (response.ok && result.success) {
        const token = result.token; // The token from the server
        ...
        } else if (result.error.includes('pass')) {
          setPasswordError(true);
